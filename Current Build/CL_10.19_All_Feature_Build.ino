...
void loop() {
  ...
  reading[6] = digitalRead(GRAIN_BUTTON);
 ...
 // Reverse Button = [5] reading
  ...
  if (reading[5] == BUTTON_ON && timeSinceLastButtonChange[5] > gateThreshold && buttonState[5] != LONG_PRESSED) {
      buttonState[5] = LONG_PRESSED;
      reverseButton = BUTTON_ON;
  }
  if (reading[5] == BUTTON_ON && timeSinceLastButtonChange[5] > debounceDelay && timeSinceLastButtonChange[5] < gateThreshold && buttonState[5] != HOLDING_PRESS) {
      buttonState[5] = HOLDING_PRESS;
      reverseButton = BUTTON_ON;
  }
  if (reading[5] == BUTTON_OFF && timeSinceLastButtonChange[5] > debounceDelay && buttonState[5] == HOLDING_PRESS) {
      buttonState[5] = SHORT_PRESSED;
  ...
  if (reading[5] == BUTTON_OFF && timeSinceLastButtonChange[5] > debounceDelay && buttonState[5] == LONG_PRESSED) {
      buttonState[5] = LONG_RELEASE;
      reverseButton = BUTTON_OFF;   
  }
   
  lastButtonState[5] = reading[5];

 // Grain Delay Button = [6] reading

  if (reading[6] != lastButtonState[6]) { // if pin state has changed from the last reading
    timeOfLastButtonChange[6] = millis(); // if pin state different, store the time of the state change
  }
  timeSinceLastButtonChange[6] = millis() - timeOfLastButtonChange[6];

  if (reading[6] == BUTTON_ON && timeSinceLastButtonChange[6] < debounceDelay) {
    // has only been low for less than the debounce time - do nothing
  }
  if (reading[6] == BUTTON_ON && timeSinceLastButtonChange[6] > gateThreshold && buttonState[6] != LONG_PRESSED) {
      buttonState[6] = LONG_PRESSED;
      grainButton = BUTTON_ON;
  }
  if (reading[6] == BUTTON_ON && timeSinceLastButtonChange[6] > debounceDelay && timeSinceLastButtonChange[6] < gateThreshold && buttonState[6] != HOLDING_PRESS) {
      buttonState[6] = HOLDING_PRESS;
      grainButton = BUTTON_ON;
  }
  if (reading[6] == BUTTON_OFF && timeSinceLastButtonChange[6] > debounceDelay && buttonState[6] == HOLDING_PRESS) {
      buttonState[6] = SHORT_PRESSED;
      if (currentShortPressState[6] == BUTTON_OFF && lastShortPressState[6] == BUTTON_ON){
      if (grainButton == BUTTON_OFF) 
      grainButton = BUTTON_ON;
      else
      grainButton = BUTTON_OFF;
  }
  lastShortPressState[6] = grainButton;
  }
  if (reading[6] == BUTTON_OFF && timeSinceLastButtonChange[6] > debounceDelay && buttonState[6] == LONG_PRESSED) {
      buttonState[6] = LONG_RELEASE;
      grainButton = BUTTON_OFF;   
  }
  ...
  if ((mode == OFF) || (mode == PASSTHROUGH)) {
    if ((recordButton == BUTTON_ON) && ((sample0Button == BUTTON_ON) || (sample1Button == BUTTON_ON) || (sample2Button == BUTTON_ON) || (sample3Button == BUTTON_ON))) {
      ...
      if ((sample0Button == BUTTON_ON) && (sampleRecorded[0])) {
        ...
        mode = PLAYBACK;
        grainAddress = 0;
        grainAddressChipNumber = playbackDirectionStart[sample];
        nSamplesPlayed = 0;
      }
 ...
 if (reverseButton == BUTTON_ON){
     ...
     playbackDirectionStart[0] = endAddress[0];
     playbackDirectionStart[1] = endAddress[1];
     playbackDirectionStart[2] = endAddress[2];
     playbackDirectionStart[3] = endAddress[3];
     }else{
     playbackDirection = 3;
