...
void loop() {
 ...
 for (int k = 5; k<7; k++) {
  ...
  if (reading[k] == BUTTON_ON && timeSinceLastButtonChange[k] > gateThreshold && buttonState[k] != LONG_PRESSED) {
  ...
  if (reading[k] == BUTTON_ON && timeSinceLastButtonChange[k] > debounceDelay && timeSinceLastButtonChange[k] < gateThreshold && buttonState[k] != HOLDING_PRESS) {
