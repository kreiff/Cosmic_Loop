...
/*
  ...
  To do: Reverse Button is hard to trigger - how to smooth?
         High Pass filter doesn't work on Reverse Playback.
         Reverse Playback triggering gets mushy / slow after 1-2 O'clock on the Pot
...
void loop() {
  ...
  reverseRead = digitalRead(REVERSE_BUTTON);

  if (reverseRead == LOW && previousState == false) {
    reverseState = true;
    previousState = true;
   }else{
    if (reverseRead == LOW && previousState == true){
      reverseState = false;
      previousState = false;
    }
   }
 ...
 if (reverseState && ((digitalRead(SAMPLE0_BUTTON) == LOW) || (digitalRead(SAMPLE1_BUTTON) == LOW) || (digitalRead(SAMPLE2_BUTTON) == LOW) || (digitalRead(SAMPLE3_BUTTON) == LOW))) {
          if ((digitalRead(SAMPLE0_BUTTON) == LOW) && (sampleRecorded[0])){
