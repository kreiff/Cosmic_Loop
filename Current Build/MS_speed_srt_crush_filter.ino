...
/*
  ...
  Bit Crushing & Sample Rate Manipulation added
...
void loop() {
  ...
  recordingSampleRate = map(analogRead(0), 0, 1023, 1000, 44100);
  recordingSampleRate = recordingSampleRate - (recordingSampleRate % 100);
  passthroughSampleRate = map(analogRead(0), 0, 1023, 1000, 44100);
  ...
  if (mode == PLAYBACK) {
    ...
    if (((sample == 0) && (digitalRead(SAMPLE0_BUTTON) == HIGH)) ||
      ...
      if (adjustablePlaybackSpeed) {
        playbackSampleRate = map(analogRead(0), 0, 1023, 1000, 44100);
      } 
