...
/*
  ...
  can be made larger or smaller by changing the Grain Size pot, and the start position can be changed on the fly by
  using the Grain Window pot.
...
//Freeze Buffer interupt implementation  
      ...
      grainAddress += grainSize;
      if (reverseButton == BUTTON_ON){
        if (grainAddress > playbackDirectionReset[sample]) {
        grainAddress = playbackDirectionStart[sample];
      }
      }else{
        if (grainAddress < playbackDirectionReset[sample]) {
