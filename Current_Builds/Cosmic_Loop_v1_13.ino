...
/*
  ...
  The grain size of the freeze window is set by the Grain Window pot (A4) and is
  automatically 15% wider than the grain-delay size to allow longer captures.
  While freeze is held, the Grain Size pot (A5) scrubs the freeze window position
  from the beginning to the end of the recorded sample.
...
// Freeze uses a wider grain window than grain-delay (115% of grainSize) so
// the user can capture longer sections of audio while freeze is engaged.
// Also computed in loop() and read-only in the ISR.
volatile unsigned int freezeGrainSize = 0;
// A5 scrub pickup: record the pot position when freeze engages so the
// playback position is not disturbed until the user deliberately moves A5.
// freezeScrubActive becomes true once A5 has moved beyond the deadband,
// at which point the scrub calculation takes over freezeAddressReset.
int  freezeEngageA5    = 0;
bool freezeScrubActive = false;
#define FREEZE_SCRUB_DEADBAND 20   // ADC counts (~2% of pot travel)
...
void loop() {
  ...
  if (mode == PLAYBACK) {
    ...
    if (freezeButton == BUTTON_ON && lastFreezeButton == BUTTON_OFF) {
      ...
      // Record A5 position at the moment of engagement so we can detect
      // deliberate movement later.  Scrub is inactive until A5 moves.
      freezeEngageA5    = potSmooth[5];
      freezeScrubActive = false;
    }
    ...
    if (((sample == 0) && (sample0Button == BUTTON_OFF)) ||
      ...
      // Freeze grain size is 115% of the grain-delay size, rounded to the
      // nearest multiple of 3 (packed SRAM stride).
      unsigned int fgs = (unsigned int)((grainSize * 115UL) / 100UL);
      fgs = fgs - (fgs % 3);
      if (fgs < 3) fgs = 3;
      freezeGrainSize = fgs;

      // While freeze is active, A5 (normally stretch) scrubs the freeze
      // window position across the full recorded sample — but only once
      // the user has deliberately moved the pot past the deadband from
      // where it was when freeze engaged.  This ensures freeze always
      // starts on the grain nearest to the last played position, not
      // wherever A5 happened to be left from a previous use.
      if (freezeButton == BUTTON_ON) {
        // Activate scrub once A5 has moved beyond the deadband.
        if (!freezeScrubActive) {
          int delta = potSmooth[5] - freezeEngageA5;
          if (delta < 0) delta = -delta;
          if (delta > FREEZE_SCRUB_DEADBAND) freezeScrubActive = true;
        }

        if (freezeScrubActive) {
          long sampleStart = (sample == 0 || sample == 2) ? 0L : 65535L;
          long sampleEnd   = endAddress[sample];
          long scrubRange  = sampleEnd - sampleStart - (long)freezeGrainSize;
          if (scrubRange < 0) scrubRange = 0;

          long scrubPos = sampleStart +
                          (long)map(potSmooth[5], 0, 1023, 0, (int)min(scrubRange, 32767L));
          // Quantise to packed-SRAM stride of 3
          scrubPos = scrubPos - (scrubPos % 3);
          if (scrubPos < sampleStart) scrubPos = sampleStart;
          if (scrubPos > sampleEnd - (long)freezeGrainSize)
            scrubPos = sampleEnd - (long)freezeGrainSize;

          freezeAddressReset = scrubPos;
        }
      }
    }
  }
...
ISR(TIMER1_OVF_vect) {
  ...
  if (mode == PLAYBACK) {
    ...
    if (evenCycle) {
      ...
      // ── Freeze mode ──────────────────────────────────────────────────────────
      if (freezeButton == BUTTON_ON) {
        // Freeze locks the window to freezeAddressReset (set on engage, then
        // scrubbed by A5 while held).  freezeGrainSize is 115% of grainSize
        // so the user has a wider capture window than grain-delay.
        grainAddress = freezeAddressReset;

        if (reverseButton == BUTTON_ON) {
          if (address <= grainAddress) address = grainAddress + freezeGrainSize - 3;
        } else {
          if (address >= grainAddress + freezeGrainSize) address = grainAddress;
        }
      }
